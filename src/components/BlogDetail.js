    ...
    {blogs.map((post, i) => (
      <div className="mt-4" key={i}>
        <div className="flex items-start">
          ...
          />
          <div>
            <Link href={`/blog/${post.id}`} className="font-semibold hover:text-[#F68921]">
              <h6>{post.title}</h6>
            </Link>
            <p className="opacity-75">{post.date}</p>
          </div>
      ...
      <ul className="flex flex-col">
        {comments.map((item, i) => (
          <li className="border-b dark:border-b-gray-600 py-2" key={i}>
            <FontAwesomeIcon icon={faComment} className="mr-2" />
            {item.author} on{' '}
            <Link href={item.link} className="font-semibold hover:text-[#F68921] transition">
              {item.title}
            </Link>
          </li>
        ))}
...
const Contents = ({ image, title, desp }) => (
  <div className="ezy__blogdetails4-contents">
    <img
      src={image}
      alt={title}
...
export async function getServerSideProps(context) {
  ...
  return {
    props: {
      blogs,
      id: parseInt(id, 10), // Parse ID to number
    },
  };
}
    ...
    <section className="ezy__blogdetails4 light pb-14 md:pb-24 bg-white dark:bg-[#0b1727] text-zinc-900 dark:text-white">
      <div className="relative w-full">
        <img
          ...
          className="sm:h-[430px] w-full object-cover"
        />
        <div className="py-4 flex justify-center items-center gap-4 px-5 absolute inset-0 text-white text-md sm:text-2xl font-bold">
          <div>
            <Link href="/" className="hover:text-[#F68921] pt-2">
              Home
            </Link>
      ...
      <div className="container px-4 py-10 sm:px-10 lg:px-16">
        <div className="grid grid-cols-12 gap-5 md:gap-6 lg:gap-10">
          ...
          <div className="col-span-12 md:col-span-8 lg:col-span-9 order-1 md:order-2">
            <Contents image={blog.image} title={blog.title} desp={blog.desp} />
          </div>
        </div>
      </div>
    </section>
